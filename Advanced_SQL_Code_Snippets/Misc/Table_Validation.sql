...
-- Insert multiple table validations by incrementing the LookupID
...
This script creates a dynamic SQL statement for performing a 
full outer join between two tables, with the ability to 
compare column counts, row counts, and distinct row counts 
between the two tables.

The script also includes the ability to join on specific columns, 
with results sorted by level of validation (100-900).
