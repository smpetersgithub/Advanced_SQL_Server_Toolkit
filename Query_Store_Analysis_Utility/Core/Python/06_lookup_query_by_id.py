...
def lookup_query_by_id(query_id, connection_string, logger, connection_timeout=10):
    ...
    Args:
        ...
        connection_timeout: Database connection timeout in seconds (default: 10)
    ...
    try:
        ...
        with pyodbc.connect(connection_string, timeout=connection_timeout) as conn:
        ...
        logger.error(f"Database error looking up query: {e}", exc_info=True)
        raise
    except Exception as e:
        logger.error(f"Unexpected error looking up query: {e}", exc_info=True)
        raise
...
def main():
    ...
    log_file = config.setup_logging('06_lookup_query_by_id')
    ...
    # Lookup query
    result = lookup_query_by_id(query_id, connection_string, logger, config.get_connection_timeout())
