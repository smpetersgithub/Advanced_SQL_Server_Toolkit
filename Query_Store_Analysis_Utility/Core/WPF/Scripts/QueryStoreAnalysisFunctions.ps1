...
function Initialize-QueryStoreAnalysis {
    ...
    # Warn for controls that could not be resolved
    ...
    if (-not $script:ReportsGrid)             { Write-Host "[WARN] Could not find 'ReportsGrid'." }
    if (-not $script:btnExecuteReport)        { Write-Host "[WARN] Could not find 'btnExecuteReport'." }
    if (-not $script:txtSelectedReport)       { Write-Host "[WARN] Could not find 'txtSelectedReport'." }
    if (-not $script:pnlProgress)             { Write-Host "[WARN] Could not find 'pnlProgress'." }
    if (-not $script:progressBar)             { Write-Host "[WARN] Could not find 'progressBar'." }
    if (-not $script:txtProgressStatus)       { Write-Host "[WARN] Could not find 'txtProgressStatus'." }
    if (-not $script:ResultsGrid)             { Write-Host "[WARN] Could not find 'ResultsGrid'." }
    ...
    if (-not $script:txtOutputLog)            { Write-Host "[WARN] Could not find 'txtOutputLog'." }
    if (-not $script:btnClearLog)             { Write-Host "[WARN] Could not find 'btnClearLog'." }
    if (-not $script:txtStatus)               { Write-Host "[WARN] Could not find 'txtStatus'." }
    if (-not $script:MainTabControl)          { Write-Host "[WARN] Could not find 'MainTabControl'." }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUkd8zl6v6exfU7wfq3C+TCUO0
# ZzSgghaOMIIDUDCCAjigAwIBAgIQJDAhS7ot/IdFcBXskCRUAjANBgkqhkiG9w0B
...
# gjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQeysO/fDMhWQsI
# hXbJzdzCzjihsTANBgkqhkiG9w0BAQEFAASCAQBKqEjKlVblHkJNhWBWEx/fn8WG
# nkYj3FnZrvBpYFLoYck1FQR/DkgFBFtUpJe36tNvzxTNBhwUunMYHxbkpPe76uaV
# /QHVOxJvcSqq+eARYb0kX516vlbW4u7mdXKDbxCdBYHbVDI/NC+yLRP1LJj1C7bn
# tvUDnaBgDehUgCcyOyGd0U4wWCko58oqiqzi8aQCHDDF/yhSf/duCoQxn4Er1T3M
# 6n3s4F1dfSIduvoUvhdWtKSYomRrnFDhV5vYSCQsby89OO0mZJ2WHmhnc1VmUYBm
# J9NzyHLp3zsJI2V91UkE0ZpgLMbrsGFYzJ75OpS1zZfQ3bXcRya8wllx6UC0oYID
...
# 9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI2MDYxOTE5Mzg0NlowLwYJKoZIhvcNAQkE
# MSIEIOUUtT8Hhr2bcKUsce12U/+ouV6LKPtP0v/Y3OtQjRkKMA0GCSqGSIb3DQEB
# AQUABIICAFYXATa9E04P6WF1CXsmyL3aDKVtEWtOMrUbQkTNQBdDaoOwPGz/1Dxh
# UXB/ZcLLlMmBxn3XE1FN82MS6IbKs8s6NUvtdOZAaKT+Zl9d0xaL+4fDEU2X2Ujp
# BYXlQ70k5dmktGutg96XMoV7cHKQ/Nd6ajS/vBXXKQ72Oa8m3xug4CssI5MW6HBp
# 06UIAAnPrmIOn6qlEhnRF7ZUGlpRcM7/6f8dmKoFER0SMEeqMmKe77yICkG7xUND
# XRxGuR1ebJqLwvk38mpu81Osxo8212Ls7rPj68oz/Mu7I1KKmMpLAIi8g7okEF4G
# yeu/1Uxw0v27Afcm2GzUta9sL7ZX6cliNG4rqQkb1m9aPXhPvOUFb9OXSrRnEFPB
# vfV0rZ+vX8AJW8uLG/CHs/LcvTiwoGiSBAUV0CPtjzMhsGckfoaDp3itriyQbWel
# okBaAnBayfpEVJu2JzQCayfy4kXd730EVUh8KqBB/Xys6U/az12rERuyrqE82Jg3
# OzrlySly1/jc5j2LvsU/UdLR34mkAYzaCnrhW3hSyS7mATJN+ZaqKMk1vpY/FmGP
# 1nC2egwUOnthXDHdd7nB4aJfOUEbz/1SpwvnfClUjgbG2V00/Ie46IYRgoyq8mwd
# 6ohfycmqZR891B3c7n+zFzx6e/FBDBdde2SieJiqBhcQFVSBWRFH
