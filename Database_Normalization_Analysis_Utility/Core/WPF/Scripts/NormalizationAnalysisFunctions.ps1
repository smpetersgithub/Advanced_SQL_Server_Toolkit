...
function Initialize-NormalizationAnalysis {
    ...
    $script:chkWindowsAuth = $MainWindow.FindName('chkWindowsAuth')
    ...
    # Warn for optional elements that may be missing
    if (-not $script:txtPrimaryKey)           { Write-Host "[WARN] Could not find 'txtPrimaryKey'." }
    if (-not $script:txtUniqueKey)            { Write-Host "[WARN] Could not find 'txtUniqueKey'." }
    if (-not $script:btnSaveConfig)           { Write-Host "[WARN] Could not find 'btnSaveConfig'." }
    if (-not $script:btnPopulateColumns)      { Write-Host "[WARN] Could not find 'btnPopulateColumns'." }
    if (-not $script:txtServer)               { Write-Host "[WARN] Could not find 'txtServer'." }
    if (-not $script:txtDatabase)             { Write-Host "[WARN] Could not find 'txtDatabase'." }
    if (-not $script:btnTestConnection)       { Write-Host "[WARN] Could not find 'btnTestConnection'." }
    if (-not $script:btnSaveConnection)       { Write-Host "[WARN] Could not find 'btnSaveConnection'." }
    if (-not $script:cmbConfigFiles)          { Write-Host "[WARN] Could not find 'cmbConfigFiles'." }
    if (-not $script:txtConfigContent)        { Write-Host "[WARN] Could not find 'txtConfigContent'." }
    if (-not $script:txtConfigFilePath)       { Write-Host "[WARN] Could not find 'txtConfigFilePath'." }
    if (-not $script:btnRefreshConfig)        { Write-Host "[WARN] Could not find 'btnRefreshConfig'." }
    if (-not $script:btnSaveConfigFile)       { Write-Host "[WARN] Could not find 'btnSaveConfigFile'." }
    if (-not $script:btnCopyConfigPath)       { Write-Host "[WARN] Could not find 'btnCopyConfigPath'." }
    if (-not $script:btnRunFullAnalysis)      { Write-Host "[WARN] Could not find 'btnRunFullAnalysis'." }
    if (-not $script:btnRunFullAnalysisConfig){ Write-Host "[WARN] Could not find 'btnRunFullAnalysisConfig'." }
    if (-not $script:btnOpenOutputFolder)     { Write-Host "[WARN] Could not find 'btnOpenOutputFolder'." }
    if (-not $script:btnClearOutputFolder)    { Write-Host "[WARN] Could not find 'btnClearOutputFolder'." }
    if (-not $script:btnClearLog)             { Write-Host "[WARN] Could not find 'btnClearLog'." }
    if (-not $script:btnOpenLogsFolder)       { Write-Host "[WARN] Could not find 'btnOpenLogsFolder'." }
    if (-not $script:progressSpinner)         { Write-Host "[WARN] Could not find 'progressSpinner'." }
    ...
    # Toggle Username/Password fields based on Windows Authentication checkbox
    if ($script:chkWindowsAuth) {
...
function Load-DatabaseConfig {
    ...
    if (Test-Path $script:DatabaseConfigPath) {
        try {
            ...
            if ($script:txtServer -and $dbConfig.servername) {
                $script:txtServer.Text = $dbConfig.servername
            }
            if ($script:txtDatabase -and $dbConfig.database) {
                $script:txtDatabase.Text = $dbConfig.database
            }
            if ($script:txtUsername -and $dbConfig.username) {
                $script:txtUsername.Text = $dbConfig.username
            }
            if ($script:txtPassword -and $dbConfig.password) {
                $script:txtPassword.Text = $dbConfig.password
            }

            # Apply Windows Authentication state
            $useWindowsAuth = $false
            if ($dbConfig.PSObject.Properties['windows_auth']) {
                $useWindowsAuth = [bool]$dbConfig.windows_auth
            }
            if ($script:chkWindowsAuth) {
                $script:chkWindowsAuth.IsChecked = $useWindowsAuth
            }
            if ($script:txtUsername) { $script:txtUsername.IsEnabled = -not $useWindowsAuth }
            if ($script:txtPassword) { $script:txtPassword.IsEnabled = -not $useWindowsAuth }
    ...
    if (Test-Path $script:TableConfigPath) {
        try {
            ...
            if ($script:txtTableName) {
                $script:txtTableName.Text = if ($tableConfig.table) { $tableConfig.table } else { "" }
            }

            # Populate columns (array to multi-line text)
            if ($script:txtColumns) {
                if ($tableConfig.columns -and $tableConfig.columns.Count -gt 0) {
            ...
            if ($script:txtPrimaryKey -and $tableConfig.primarykey) {
            ...
            if ($script:txtUniqueKey -and $tableConfig.uniquekey) {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUdLnuRdYFQDQQ9C1fhc1jEqgN
# JGKgghaOMIIDUDCCAjigAwIBAgIQJDAhS7ot/IdFcBXskCRUAjANBgkqhkiG9w0B
...
# gjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBTnKt+okI8wdSC7
# DC4hmURh4ohnXTANBgkqhkiG9w0BAQEFAASCAQBcoUZ5vE71qEUI96sx95T3rTdb
# SsCXyOYBYI6jAt/oh0OTk3s8mk0MfP5bdf+0SxULJLZLKxMu3NhKsMJV9zCUXU/o
# NMlEZJ/cnJjkhwi/2jTWXLWUIKFNicOhDCqqQfAktM7sIn2WKDsznRwdygfbGYVf
# Iqi7dKtObFXfeAr4eswM70mNo7cwIOpaGmktFKxYXIJF5xHc2lANjmKV8CkLBmN/
# ihKxfvv9c+LLSrS6lXP+M1RtZuxL16pl9Dkk1+RJ/eqIYegLbO9Mf/3ZaQsQQ53p
# KVlwZ8kkOQSLpvk02RZCpwpzE9axwLHpkJa5KO4ms5TxASMWmtI24IPkgiUnoYID
...
# 9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI2MDYyNjE1MjIyMlowLwYJKoZIhvcNAQkE
# MSIEIMxSEQxKmlPC9YHjBp2K8QeJJcNNw9mPa05ZZ1NWsmaZMA0GCSqGSIb3DQEB
# AQUABIICAD7Y030IVcqhXTxkhGg1rowdsjCzx2tj2W17I61QePB+iJjsIn93mABA
# QUWApX0mJJyQ8zyGz9jkcZrFpbzqM+90mlkpEaal7tLMxFIbuQR2V7uAZIgDAKKL
# dlkJ7n5g4HH1Q0Zs3pDUxp/Rq9mM5fSCh5S01oFZtdLzPrqrR3qdOJvZg2oiSqbO
# GVHtuKr3RfnZuaWothC/rrNwxtxJzzzxaQde2xeC+05qQs1JJALR/ohKTD1puArP
# M2C2oYpv1NKx8SgliG14vYFgRz0LBBZMN8vmkHmz1s4HkhaX8aVbN75NVD8kNGLF
# Gex7711tKk9IAZd6QWvn4rmwXvP427CVR86MzveH2iyTVAwmM4fcGVl6x0NFnnWX
# pUC8jk1R82+Ds3cbaFh6kOU4TFMHAbHQYQB1POtOVizVd0KQZ1zl0VNDpEZPBqI9
# D3xtao8oOGdPu1zDx1uWT1ixzYiW2ER+gvAsdJn4hPNo6Sj3LAKbXARqX/G3g4Qj
# Ff6HVEPTw+rKPLFu0nZgvf1LrL2eJZf6Cfr8glfgWqVa8sQE6AsHENE6KjAeOB3y
# fh46zvLUDvShnUVZUu1wYuIFAIPHOIwzdTU87sxuyEnkvAZIX5EKH9COlRH/8XJi
# Y0s1nDQJfJKVvbG/GhyDzcDI72C+/2ew9YuxMTbRN/qBGY4cWQak
