...
set "OUTPUT_EXE=%BUILD_DIR%\Database Normalization Analysis Utility.exe"
set "ICON_PATH=%WPF_DIR%\Assets\icons8-goldfish-ico.ico"
set "XAML_FILE=%SCRIPTS_DIR%\MainWindow.xaml"

set "SHORTCUT_NAME=Database Normalization Analysis Utility.lnk"
set "SHORTCUT_SOURCE=%BUILD_DIR%\%SHORTCUT_NAME%"
set "SHORTCUT_DEST=%PROJECT_ROOT%\%SHORTCUT_NAME%"

:: --- Kill EXE if running ---
taskkill /IM "Database Normalization Analysis Utility.exe" /F >nul 2>&1
timeout /t 1 >nul
