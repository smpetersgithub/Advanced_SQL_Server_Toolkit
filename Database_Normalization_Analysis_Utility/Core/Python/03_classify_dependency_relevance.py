...
def load_results(results_path):
    ...
    try:
        ...
        logger.error("Please run the 'Analyze Functional Dependencies' script first.")
        sys.exit(1)
...
def update_results_with_classification(results):
    ...
    logger.info("=" * 80)
    logger.info("Classifying Functional Dependencies by Relevance")
    logger.info("=" * 80)
    ...
    # Print summary
    logger.info("Relevance Classification Summary:")
    logger.info("-" * 80)
...
def save_results(results, output_path):
    ...
    try:
        ...
        logger.error(f"Cannot write to file {output_path}: {e}")
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        sys.exit(1)
    log_file = config.setup_logging('03_classify_dependency_relevance')
