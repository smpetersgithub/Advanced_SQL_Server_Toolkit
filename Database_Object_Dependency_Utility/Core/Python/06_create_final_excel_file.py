...
def main():
    ...
    # Setup logging
    log_file = config.setup_logging('06_create_final_excel_file')

    logger.info("="*60)
    logger.info("Creating Final Excel Report")
    logger.info("="*60)
    logger.info(f"CSV Input:     {csv_file}")
    logger.info(f"Forward JSON:  {forward_json}")
    logger.info(f"Reverse JSON:  {reverse_json}")
    logger.info(f"Excel Output:  {output_file}")
    logger.info(f"Java Source Directories:")
    for src_dir in java_source_dirs:
        logger.info(f"  - {src_dir}")
    logger.info("")

    # Create Excel report
    success = create_excel_report(csv_file, forward_json, reverse_json, output_file, java_source_dirs, logger)
