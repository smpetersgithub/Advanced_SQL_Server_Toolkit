...
def scan_dao_file(filepath, dao_count_tracker):
    ...
    try:
        ...
        logging.warning(f"Failed to scan DAO file {filepath}: {e}")
...
def scan_ui_file(filepath):
    ...
    try:
        ...
        logging.warning(f"Failed to scan UI file {filepath}: {e}")
...
def main():
    ...
    logging.info("=" * 80)
    logging.info("Database Object Dependency Utility - Complete UI Mapping Generator")
    logging.info("Stored Procedure -> DAO -> UI Handler/Action -> JSP")
    logging.info("=" * 80)

    # Validate configuration
    if not java_source_dirs:
        logging.warning("No Java source directories configured in config.json")
        logging.warning("Please add 'java_source_dirs' to the 'paths' section")
