...
def main():
    ...
    # Setup logging
    log_file = config.setup_logging('04_generate_dependency_report_reverse')

    # Load stored procedures
    logger.info("Loading database objects...")
    ...
    logger.info(f"Found {len(procedures)} database objects to process")
    for p in procedures:
        logger.info(f"  Object: {p}")

    # Validate we have procedures to process
    if not procedures:
        logger.warning("No database objects found in input file")
        logger.info("Exiting - nothing to process")
        return

    # Load SQL script
    logger.info("Loading reverse dependency SQL script...")
