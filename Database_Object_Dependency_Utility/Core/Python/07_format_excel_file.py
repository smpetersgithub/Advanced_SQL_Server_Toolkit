...
def main():
    ...
    # Setup logging
    log_file = config.setup_logging('07_format_excel_file')

    logger.info("="*60)
    logger.info("Formatting Excel Report")
    logger.info("="*60)
    logger.info(f"Input file:                  {input_file}")
    logger.info(f"Output file:                 {output_file}")
    logger.info(f"Naming convention:           {convention}")
    logger.info(f"Remove object descriptions:  {remove_descriptions}")

    if convention == 1:
        logger.info("  Format: object_name only")
    elif convention == 2:
        logger.info("  Format: schema.object_name")
    elif convention == 3:
        logger.info("  Format: database.schema.object_name (full)")

    logger.info("")

    # Format Excel file
    success = format_excel_file(input_file, output_file, convention, remove_descriptions, logger)
    ...
    if success:
        ...
        logger.info("="*60)
        logger.info(f"Formatted Excel file: {output_file}")
        logger.info(f"Log file:             {log_file}")
    else:
        logger.error("Failed to format Excel file")
        sys.exit(1)
