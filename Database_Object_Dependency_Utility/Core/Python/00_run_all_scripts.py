    ...
    try:
        ...
        for item in items:
            try:
                ...
                logger.error(f"  ERROR deleting {item.name}: {e}", exc_info=True)
                return False
        ...
        logger.error(f"ERROR: Failed to clean output directory: {e}", exc_info=True)
        return False
...
def run_script(script_name, script_dir, logger):
    ...
    try:
        ...
        logger.error(f"\n✗ ERROR running {script_name}: {e}", exc_info=True)
        return False
