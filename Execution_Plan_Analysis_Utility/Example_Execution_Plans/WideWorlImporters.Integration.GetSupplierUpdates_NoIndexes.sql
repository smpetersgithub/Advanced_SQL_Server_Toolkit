...
/****** Object:  StoredProcedure [Integration].[GetSupplierUpdates_NoIndexes]    Script Date: 12/22/2025 ******/
SET ANSI_NULLS ON
GO
...
-- Demo version that forces table scans (no index usage) for execution plan comparison


--EXECUTE [Integration].[GetSupplierUpdates_NoIndexes] '01/01/1900','12/31/9999'


CREATE PROCEDURE [Integration].[GetSupplierUpdates_NoIndexes]
...
BEGIN
    ...
    -- MODIFIED: Added INDEX(0) hints to force table scans

    DECLARE SupplierCategoryChangeList CURSOR FAST_FORWARD READ_ONLY
    FOR
    SELECT cc.SupplierCategoryID,
           cc.ValidFrom
    FROM Purchasing.SupplierCategories_Archive AS cc WITH (INDEX(0))
    ...
    SELECT cc.SupplierCategoryID,
           cc.ValidFrom
    FROM Purchasing.SupplierCategories AS cc WITH (INDEX(0))
    WHERE cc.ValidFrom > @LastCutoff
    AND cc.ValidFrom <= @NewCutoff
    AND cc.ValidFrom <> @InitialLoadDate
    ORDER BY ValidFrom;
    ...
    BEGIN
        ...
        FROM Purchasing.Suppliers FOR SYSTEM_TIME AS OF @ValidFrom AS s WITH (INDEX(0))
        INNER JOIN Purchasing.SupplierCategories FOR SYSTEM_TIME AS OF @ValidFrom AS sc WITH (INDEX(0))
        ON s.SupplierCategoryID = sc.SupplierCategoryID
        INNER JOIN [Application].People FOR SYSTEM_TIME AS OF @ValidFrom AS p WITH (INDEX(0))
    ...
    SELECT c.SupplierID,
           c.ValidFrom
    FROM Purchasing.Suppliers_Archive AS c WITH (INDEX(0))
    WHERE c.ValidFrom > @LastCutoff
    AND c.ValidFrom <= @NewCutoff
    UNION ALL
    SELECT c.SupplierID,
           c.ValidFrom
    FROM Purchasing.Suppliers AS c WITH (INDEX(0))
    WHERE c.ValidFrom > @LastCutoff
    AND c.ValidFrom <= @NewCutoff
    ORDER BY ValidFrom;
    ...
    -- MODIFIED: Removed index creation on temp table to avoid any index usage
